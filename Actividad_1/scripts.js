...
class Carrito{
  ...
  btnComprar.addEventListener("click", () => {
    const total = parseFloat(totalFinal.textContent.replace("€", "").trim());

    if (total > 0) {
      alert("✅ ¡Compra realizada con éxito!");
      // Vaciar la cesta después de comprar
      productosCarrito.innerHTML = "";
      totalFinal.textContent = "0€";
    } else {
      alert("⚠️ No hay productos en la cesta");
    }
  });
...
const cargarProductos = (productos) => {
    // Ahora tomamos los datos dentro de 'record'
    if (!productos || !productos.record || !productos.record.products) {
        console.error("JSON inválido o productos no encontrados", productos);
        return;
    }

    misProducts = productos.record.products;
    currency = productos.record.currency;
...
fetch("https://api.jsonbin.io/v3/b/68e2a9a5d0ea881f409652a8")
  .then(res => res.json())
  .then(productos => {
      console.log("Productos cargados:", productos); // Para depurar
